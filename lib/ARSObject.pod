...
C<connect>;
C<-schema>, C<-meta>, C<-metax>, C<-metadn>, C<-metaid>, C<-schgen>, C<-schfdo>, C<-strFields>;
...
=item soon (minutes, logfile ||'', run command || [command line] || sub{}, soon command || [command line])

=item soon (minutes, logfile ||'', run command || [command line] || sub{})

=item soon (minutes, logfile ||'', '', soon command || [command line])
...
implemented tet/binary format autodetection.
